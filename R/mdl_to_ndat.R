...
#'
#' @param mdl A GAM object (gam or bam), from which you would like to obtain predictions.
#' @param target A character vector, which indicates variables to vary.
#' @param len A numeric, which specifies length of each varied variable, when the variable is numeric.
#' @param method A function that takes a vector of numerics and returns a single value (e.g., median as default). The function given through this argument is used to produce a constant value for each of the numeric variables you would like to keep constant.
#' @return A data.frame with the variables specified by "target" being varied and the other variables being kept constant. The returnd data.frame should be ready to be used for, e.g., predict.gam.
