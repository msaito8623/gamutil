...
tmdl0  <- gam(y ~ s(x0, by=fac) + s(x1, by=fac, k=5) + s(x2)
	        + ti(x0,x1,by=fac) + ti(x0,x2,by=fac), data=tdat)
x2max <- max(tmdl0$model$x2)

tmdl1  <- gam(y ~ s(x0), data=tdat)

tmdl2 <- gam(y ~ s(x0, by=fac) + s(x1, by=fac, k=5) + ti(x0,x1), data=tdat)
tdat3 <- tdat
set.seed(432)
tdat3$foo <- factor(sample(LETTERS[1:3], nrow(tdat3), replace=TRUE))
tmdl3 <- gam(y ~ s(x0, by=fac) + s(x1, by=foo) + ti(x0,x1, by=fac), data=tdat3)
...
### Output ###
usethis::use_data(tdat, tdat3, tmdl0, tmdl1, tmdl2, tmdl3, x2max, mdl,
		  mdl_fac, internal=TRUE, overwrite=TRUE)
